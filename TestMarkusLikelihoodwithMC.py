...
parser.add_option("-r", "--regprior", action="store", type="float", default=10.0, dest="REGPRIOR", help="Prior for the regular component")
parser.add_option("-d", "--randprior", action="store", type="float", default=10.0, dest="RANDPRIOR", help="Prior for the regular component")
parser.add_option("-i", "--inputfule", action="store", type="string", default=None, dest="INFILE", help="Input file of UHECRs, if random generation is to be skipped")
parser.add_option("-m", "--mc", action="store", type="int", default=1, dest="MC", help="If not one, MC datasets are produced in this number and the likelihood is not applied")
parser.add_option("-f", "--outputfolder", action="store", type="string", default='', dest="OUTFOLD", help="Input file of UHECRs, if random generation is to be skipped")
...
REGPRIOR = options.REGPRIOR
RANDPRIOR = options.RANDPRIOR
INFILE = options.INFILE
MCONLY=options.MC
OUTFOLD = options.OUTFOLD
...
def LH(x) :
	...
	priors = (B0*180./np.pi/REGPRIOR)**2 + (thetascale/RANDPRIOR)**2
...
for counter in range(seqno, seqno+MCONLY):

    fname = str(massno)+'_'+str(atomicno)+'_p'+str(psno)+'_u'+str(uhecrno)+'_E'+str(ethresh)+'_'+str(counter)
    ...
    if INFILE is None:
        ...
        plt.savefig(OUTFOLD+'PS_'+fname+'.png')
        if weightflag:
            np.savetxt(OUTFOLD+'PS_'+fname+'.txt', np.vstack([psras, psdecs, z]).transpose(), delimiter='|')
        else:
            np.savetxt(OUTFOLD+'PS_'+fname+'.txt', np.vstack([psras, psdecs]).transpose(), delimiter='|')
        del plt
        import matplotlib.pyplot as plt

        plot_mwd(pickedra, pickeddec, title = 'UHECRs')
        #plt.show()
        plt.savefig(OUTFOLD+'UHECR_'+fname+'.png')
        np.savetxt(OUTFOLD+'UHECR_'+fname+'.txt', pickedarr.transpose(), delimiter='|')
    else:
        pickedarr = np.genfromtxt(INFILE, delimiter='|').transpose()
        pickedcoord = SkyCoord(l = pickedarr[1]*u.radian, b = pickedarr[0]*u.radian, frame='galactic')
        pickedra = pickedcoord.icrs.ra.value
        pickeddec = pickedcoord.icrs.dec.value
        print 'GeneratedUHECRs:', pickedarr
        fname=INFILE.replace('.txt','REDONE')
        #plt.show()
...
if MCONLY==1:
    ...
    np.savetxt(OUTFOLD+'maps_LH'+fname+'nside32.txt', np.vstack([map, mapns, maptheta, mapx, mapy, mapz]), delimiter='|')
